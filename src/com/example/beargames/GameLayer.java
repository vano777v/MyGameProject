...
{
	protected static CGSize screenSize;
    ...
    protected static int[] buffer_team;
    protected static int[] buffer_ability_items={0,0,0};
    private static int[] buffer_ability_team = {4,6,10,16};
	//private CCMenu gen = null;
	 static CCProgressTimer item1;
	protected CCBitmapFontAtlas statusLabel;
	//private int istouch =0;
	private  Game_Arena arena = null;
	protected  int  last_touch_ability=0;
   ...
   // private CCSprite m, n = null; 
	...
	protected ArrayList<MenuLayer> menus_game= null;
	  float vp=0 ,dt=0,pz=0;
	 protected Constants_Game const_level= null;
	 abstract public Main_Personage bear_build(int tag);
	 abstract public Boolean menu_control(MotionEvent event);
	 abstract public void ability_menu_init();
	 abstract public void button_ability_progress_action(int deter_path_id, int unpress_id);
    public GameLayer( String _campaign, Game_Arena arena, Main_Load_Screen _load)
    {
    ...
    private void bear_team_init(Boolean blank_game, int menu_index)
    {
    	if(blank_game)
    	{
    		...
    		while(i<=8)
    		{
    			...
    			path = campaign+"menus/block/"+(i)+"b.png";
    ...
    protected int trans=0;
     public boolean ccTouchesEnded(MotionEvent event) 
     {
    	...
    	Boolean result=menu_control(event); 
    ...
    protected void  button_bear_team_item_select(int menu_index, int bear_menu_index ,int index, int item_tag, float time)
     {
    	...
    	if(menus_game.get(menu_index).get_item(index).get_touch_state())
    	 ...
    	 else
    	 {
    		 ...
    		 menus_game.get(menu_index).change_Image_item(campaign+"menus/block/"+item_tag+"b.png", index);
    		 ...
    		 menus_game.get(menu_index).get_item(index).time_progress_bar_init(campaign+"menus/choosed/"+item_tag+"a.png", CCProgressTimer.kCCProgressTimerTypeVerticalBarBT);
    		 menus_game.get(menu_index).get_item(index).set_path_progress_bar(campaign+"menus/neutral/"+item_tag+"n.png");
    		 ...
    		 menus_game.get(bear_menu_index).change_Image_item(campaign+"menus/press/"+item_tag+"p.png", item_tag*2);
    		 team_selected++;
    		 ...
    		 if((team_selected-menus_game.get(menu_index).get_item(index).team_count)==4||(team_selected-menus_game.get(menu_index).get_item(index).team_count)==0)
    		 {
    			 for(int i=2;i<18;i+=2)
    			 {
    			   if(menus_game.get(bear_menu_index).get_item(i).get_isTouchEnabel()&& !menus_game.get(bear_menu_index).get_item(i).get_touch_state()){	 
    				  menus_game.get(bear_menu_index).change_Image_item(campaign+"menus/neutral_press/"+(i/2)+"np.png", i);
     ...
     protected void button_ability_item_press(int index, int menu_index)
     {
    	 String path=campaign+"menus/ability_icons/"+last_touch_ability+"ab_i/1a.png";
    	 ...
    	 path=campaign+"menus/ability_icons/"+index+"ab_i/1p.png";
    	 ...
    	 for(int i=0;i<3;i++)
    	 {
    	 	...
    	 	if(i==0)
    	 	{
    	 		 menus_game.get(menu_index).change_Image_item(campaign+"menus/ability_icons/"+index+"ab_i/"+index+"_mb_a.png", i+1);
    	    	
    	 	}else
    	 	{
    	 		 menus_game.get(menu_index).change_Image_item(campaign+"menus/ability_icons/"+index+"ab_i/"+(i+1)+"a.png", i+1);
    	 	}
     ...
     private void main_menu_init(float percents)
     {
    	 ...
    	 MenuLayer main_menu =   new MenuLayer(ccColor4B.ccc4(255,255, 255,255),campaign,campaign+"menus/menu2.png",2, CGSize.make(3048f, 250f), perc);
    	 main_menu.setPosition(0,0);
    	 main_menu.setIsTouchEnabled(true);
    	 main_menu.add_item(campaign+"menus/rim_stone.png", 1, CGPoint.make(360, 10), CGSize.make(200, 200));
         main_menu.add_item(campaign+"menus/strategy_unpress.png",2, CGPoint.make(370, 20), CGSize.make(180, 180));
         main_menu.add_item(campaign+"menus/rim_stone.png",3, CGPoint.make(20, 10), CGSize.make(200, 200));
         main_menu.add_item(campaign+"menus/main_unpress.png",4, CGPoint.make(30, 20), CGSize.make(180, 180));
         ...
         for(int i=5;i<=12;i++)
         {
        	 main_menu.add_item(campaign+"menus/rim.png", i, CGPoint.make(dist, 10), CGSize.make(200, 200));
        	 i++;
        	 count=i/2-2;
             main_menu.add_item(campaign+"menus/neutral/"+count+"n.png", i, CGPoint.make(dist+10, 20), CGSize.make(180, 180));
     ...
     protected int press_id_ability=0;
     protected int unpress_id_ability=0;
	public Button_Action button_ability_action=null;
    
     
     protected void ablity_items_init(MenuLayer ability_menu , int bear_menu_index)
     {
    	float coord_x=80, coord_y=752;
    	int j=0, count=1, count_bear_main=0;
    	String path=null;
    	ability_menu.add_item(campaign+"menus/ability_icons/5ab_i/5_mb_a.png", 1, CGPoint.make(20, 977), CGSize.make(225, 225));
    	ability_menu.get_item(1).setisTouchEnabled(true);
    	ability_menu.add_item(campaign+"menus/ability_icons/5ab_i/2a.png", 2, CGPoint.make(320, 1017), CGSize.make(150, 150));
    	ability_menu.get_item(2).add_button_action_activity(button_ability_action);
    	ability_menu.get_item(2).setisTouchEnabled(true);
    	ability_menu.add_item(campaign+"menus/ability_icons/5ab_i/3a.png", 3, CGPoint.make(570, 1017), CGSize.make(150, 150));
    	ability_menu.get_item(3).add_button_action_activity(button_ability_action);
    	 ...
    	 for(int i=4;i<20;i+=4)
    	{
    	 ...
    	 if(menus_game.get(bear_menu_index).get_item(buffer_ability_team[count_bear_main]).get_isTouchEnabel()){	 
    		 ability_menu.add_item(campaign+"menus/rim.png", i, CGPoint.make(coord_x,coord_y), CGSize.make(160, 160));
    		 j++;
    		 coord_x+=10;
    		 coord_y+=10;
    		 path=campaign+"menus/ability_icons/"+j+"ab_i/"+count;
    	 ...
    	 for(int i=20;i<24;i++)
    	 {
    		 ability_menu.add_item(campaign+"menus/ability_icons/glass.png", i, CGPoint.make(285, coord_y), CGSize.make(521, 160));
    		 coord_y-=240;
    	 }
     ...
     private void bears_menu_init( CGSize screen_size, CGSize percents)
     {
    	...
    	// CGSize scale_factors = CGSize.make(1, 1); 
    	 ...
    	 MenuLayer bears_menu =  new MenuLayer(ccColor4B.ccc4(255,255, 255,255),campaign,campaign+"menus/menu1.png",1, CGSize.make(800f, 1082f), s);    	
    	...
    	float sizex = bears_menu.get_size().width/800f;
    	 ...
    	 for(int i=1;i<17;i++)
    	 {
    		 ...
    		 bears_menu.add_item(campaign+"menus/rim.png", i, CGPoint.make(coordx, coordy), CGSize.make(160f, 160f));
    		 i++;
    		 bears_menu.add_item(campaign+"menus/neutral/"+(i/2)+"n.png", i, CGPoint.make(coordx+10, coordy+10), CGSize.make(140f, 140f));
    		 bears_menu.get_item(i).setisTouchEnabled(true);
    		 java.lang.System.out.println(campaign+"menus/neutral/"+i+"n.png");
    		 count++; 
    		 coordx+=370;
    		 if(count==3)
    		 {
     ...
     private void top_menu_init ()
     {
    	...
    	final  MenuLayer top_menu =  new MenuLayer(ccColor4B.ccc4(255,255, 255,255),campaign,campaign+"menus/menu4.png",5, CGSize.make(3840f, 188f), perc);
    	 ...
    	 top_menu.add_item(campaign+"menus/rim_stone.png", 1, CGPoint.make(orig_x+20,12f), CGSize.make(170f, 170f));
    	 top_menu.add_item(campaign+"menus/rim_stone.png", 2, CGPoint.make(3650f,12f ), CGSize.make(170, 170));
    	 
    	 top_menu.add_item(campaign+"menus/settings_items/zoom_in_unpress.png", 3, CGPoint.make(orig_x+30,22f ), CGSize.make(150, 150));
    	 top_menu.add_item(campaign+"menus/settings_items/zoom_out_unpress.png", 4, CGPoint.make(3660f,22f ), CGSize.make(150, 150));
    ...
    private void setting_menu_init ()
     {
    	 ...
    	 float perc = screenSize.height/1600f;
    	 final MenuLayer setting_menu =  new MenuLayer(ccColor4B.ccc4(255,255, 255,255),campaign,campaign+"menus/menu5.png",4, CGSize.make(762f, 754f), perc);
    	setting_menu.setOpacity(0);
    	 setting_menu.setPosition(screenSize.width-975f*perc,screenSize.height-188f*perc);
    	 setting_menu.add_item(campaign+"menus/settings_items/sound_on.png", 1, CGPoint.make(93,270 ), CGSize.make(150, 150));
    	 setting_menu.get_item(1).setisTouchEnabled(true);
    	 setting_menu.add_item(campaign+"menus/settings_items/restart_unpress.png", 2, CGPoint.make(343,270 ), CGSize.make(150, 150));
    	 setting_menu.get_item(2).setisTouchEnabled(true);
    	 setting_menu.add_item(campaign+"menus/settings_items/save_unpress.png", 3, CGPoint.make(588,270 ), CGSize.make(152, 150));
    	 setting_menu.get_item(3).setisTouchEnabled(true);
    	 setting_menu.add_item(campaign+"menus/settings_items/pause.png", 4, CGPoint.make(588,23 ), CGSize.make(152, 150));
    	 setting_menu.get_item(4).setisTouchEnabled(true);
    	 setting_menu.add_item(campaign+"menus/settings_items/stop_unpress.png", 5, CGPoint.make(588,518 ), CGSize.make(152, 150));
     ...
     protected void button_item_bears_press(int item_index, int menu_tag, int menu_main_index)
     {
    	 ...
    	 {
    		path =  campaign+"menus/neutral/"+(item_index/2)+"n.png";
    	 ...
    	 {
    		 path = campaign+"menus/choosed/"+(item_index/2)+"a.png";
    			 ...
    			 else
    			 {
    			  ...
    			  if(search_not_progress_place(menu_main_index)!=-1){	 
    				int tag= buffer_team[(search_not_progress_place(menu_main_index)-6)/2];
    				 ...
    				 path = campaign+"menus/neutral/"+(tag/2)+"n.png";
      ...
      private void update_control_team(int menu_index)
     {
    	 ...
    	 for(int i=0;i<buffer_team.length;i++)
    	 {
    		 ...
    		 {
    			 // 
    			path=campaign+"menus/neutral/"+Integer.toString(buffer_team[i]/2)+"n.png";
    			menus_game.get(menu_index).change_Image_item(path, i*2+6);
    			path=campaign+"menus/choosed/"+Integer.toString(buffer_team[i]/2)+"a.png";
    			menus_game.get(menu_index).get_item(i*2+6).time_progress_bar_init(path, CCProgressTimer.kCCProgressTimerTypeVerticalBarBT);
     ...
     protected void button_bears_choose( int menu_index, int menu_move, int item_index)
     {
    	 ...
    	 if(menus_game.get(menu_index).get_item(item_index).get_touch_state()){ 
    		 ...
    		 menus_game.get(menu_index).get_item(item_index).set_touch_state(false);
    		  ...
    		  menus_game.get(menu_index).change_Image_item(campaign+"menus/main_unpress.png", item_index);
    		...
    		else
    		{
    			  ...
    			  // SoundEngine.sharedEngine().playEffect(mMyApp.getApplicationContext(), raw.bear_growl_001);
    			   menus_game.get(menu_index).change_Image_item(campaign+"menus/main_press.png", item_index);
     ...
     protected void button_zoom_out (int menu_index, int item_tag)
     {
    	if(count_zoom<3&&count_zoom>0)
    	{
    		
    	  menus_game.get(menu_index).get_item(item_tag).button_press(campaign+"menus/settings_items/zoom_out_unpress.png",campaign+"menus/settings_items/zoom_out_press.png", 0.2f);
    	  scale_factor/=1.2f;
     ...
     protected void button_zoom_in (int menu_index, int item_tag)
     {
        ...
        // level.zoom_in();
    	 if(count_zoom<2&&count_zoom>=0)
    	 {
    		 ...
    		 menus_game.get(menu_index).get_item(item_tag).button_press(campaign+"menus/settings_items/zoom_in_unpress.png",campaign+"menus/settings_items/zoom_in_press.png", 0.2f);
    		scale_factor *=1.2f;
    		//arena.get_Main_Base_list(0).get_animation_element(0).start_action(0);
      ...
      protected void button_ability_press(int menu_index, int menu_item)
     {
    	   if(!menus_game.get(menu_index).get_item(menu_item).get_touch_state()){
    		   menus_game.get(menu_index).change_Image_item(campaign+"menus/ability_icons/"+buffer_ability_items[0]+"ab_i/"+buffer_ability_items[0]+"_mb_p.png",menu_item );
    	   ...
    	   {
    		   menus_game.get(menu_index).change_Image_item(campaign+"menus/ability_icons/"+buffer_ability_items[0]+"ab_i/"+buffer_ability_items[0]+"_mb_a.png",menu_item );
      ...
      protected void button_stop_press(int menu_index, int item_tag )
      {
    	  menus_game.get(menu_index).get_item(item_tag).button_press(campaign+"menus/settings_items/stop_unpress.png", campaign+"menus/settings_items/stop_press.png", 0.2f);
      ...
      protected void button_sound_press (int menu_index, int item_tag)
      {
    	  if(menus_game.get(menu_index).get_item(item_tag).get_touch_state()){ 
     		 ...
     		 menus_game.get(menu_index).change_Image_item(campaign+"menus/settings_items/sound_on.png", item_tag);
     		...
     		else
     		{
     			menus_game.get(menu_index).get_item(item_tag).set_touch_state(true);
       		    menus_game.get(menu_index).change_Image_item(campaign+"menus/settings_items/sound_off.png", item_tag);
       		    statusLabel.setString(Float.toString(41f));
       		// SoundEngine.sharedEngine().playEffect(mMyApp.getApplicationContext(), R.raw.button_press);
     ...
     protected void button_pause_press(int menu_index, int item_index) 
     {
    	 float coord_x= menus_game.get(menu_index).getPosition().x;
    	 if(menus_game.get(menu_index).get_item(item_index).get_touch_state()){ 
    		   
    		 menus_game.get(menu_index).get_item(item_index).set_touch_state(false);
    		 menus_game.get(menu_index).change_Image_item(campaign+"menus/settings_items/pause.png", item_index);
    		...
    		{
    			menus_game.get(menu_index).get_item(item_index).set_touch_state(true);
      		  	menus_game.get(menu_index).change_Image_item(campaign+"menus/settings_items/play.png", item_index);
     ...
     protected void button_restart_press(int menu_index, int item_tag)
     {
    	 menus_game.get(menu_index).get_item(item_tag).button_press(campaign+"menus/settings_items/restart_unpress.png", campaign+"menus/settings_items/restart_press.png", 0.2f);
    	 //SoundEngine.sharedEngine().playEffect(mMyApp.getApplicationContext(), R.raw.button_press);
     }
     protected void button_save_press(int menu_index, int item_tag)
     {
    	 menus_game.get(menu_index).get_item(item_tag).button_press(campaign+"menus/settings_items/save_unpress.png", campaign+"menus/settings_items/save_press.png", 0.2f);
    	 //SoundEngine.sharedEngine().playEffect(mMyApp.getApplicationContext(), R.raw.button_press);
     }
